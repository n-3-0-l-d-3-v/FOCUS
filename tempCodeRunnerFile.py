...
class GestureDetector:
    ...
    def stop(self):
        self.running = False
        # FIX: Add error handling to prevent crash on exit
        try:
            if self.face_mesh:
                self.face_mesh.close()
        except ValueError:
            # This can happen if MediaPipe's graph is already closed. Safe to ignore.
            pass
...
class GameWidget(FloatLayout):
    def __init__(self, is_first_launch=True, **kwargs):
        ...
        with self.canvas.before:
            Color(0.05, 0.05, 0.1, 1) # Dark blue-black background
            self.bg_rect = Rectangle(size=self.size, pos=self.pos)
        self.bind(size=self._update_bg_rect, pos=self._update_bg_rect)
        self.bind(size=self._update_borders) # FIX: Bind border update to size change
    ...
    # FIX: New method to dynamically update border characters
    def _update_borders(self, instance, value):
        # Estimate how many characters fit based on font size and window width
        # This is an approximation but works well for monospaced fonts
        char_width = self.top_border.font_size * 0.5 
        num_chars = int(self.width / char_width) - 4 # Subtract for corners and padding
    ...
    def build_ui(self):
        ...
        self.loading_layout.add_widget(Widget(size_hint_y=0.2)) # Spacer
        ...
        # Borders are now initialized empty and updated dynamically
    ...
    def set_hud_visibility(self, is_visible):
        self.top_panel.opacity = 1 if is_visible else 0
        self.bottom_panel.opacity = 1 if is_visible else 0
    ...
    def generate_new_rule(self):
        ...
        rule_text = " | ".join([f"{s} -> {g.replace('_', ' ')}" for s, g in self.current_rule.items()])
        self.rule_label.text = rule_text
        
        instr_text = "--- CONTROLS ---\n\n"
        instr_text += "\n".join([f"{s}  ->  {g.replace('_', ' ')}" for s, g in self.current_rule.items()])
        instr_text += "\n\nStarting in..."
    ...
    def update(self, dt):
        ...
        # FIX: Make sync zone width responsive to screen size
    ...
    def spawn_symbol(self):
        ...
        widget = Label(text=symbol_text, font_name=config.FONT_NAME, font_size='40sp',
                       size_hint=(None, None), size=('80sp', '80sp'),
                       pos_hint={'center_y': 0.5}, x=-80)
    ...
    def frame_from_texture(self, texture):
        ...
        cv2_image = cv2.cvtColor(pil_image, cv2.COLOR_RGBA2BGR)
        return cv2_image
